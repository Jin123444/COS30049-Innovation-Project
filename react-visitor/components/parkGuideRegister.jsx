...
const validateForm=async()=>
{
  ...
  if(!repeatPassword)
    ...
    if (Object.keys(errors).length === 0) {
      try {
        const response = await fetch('http://192.168.0.105:5000/api/parkGuideRegister', {
          method: 'POST',
        ...
        if (response.ok) {
          alert("Park Guide registered successfully!");
          navigate("/login");
        } else {
  ...
  return (
    <div style = {styles.container}>
        <h2 style={styles.title}>Park Guide Register</h2>
        ...
        <label style={styles.form_label}>Fullname</label>
        <input
            type = "text"
            style = {styles.form_input}
        ...
        />
        {
            errors.fullname?<label style={styles.error}>
                {errors.fullname}</label>: null
        }
...
export default ParkGuideRegister;